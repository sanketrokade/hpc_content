...
CXX="${CXX:-g++}"; MPICXX="${MPICXX:-mpicxx}"
...
if command -v "$CXX" >/dev/null; then
    ok "$($CXX --version | head -1)"
    printf '#include <pthread.h>\nint main(void){pthread_barrier_t b;(void)b;return 0;}\n' > "$tmp/p.cpp"
    "$CXX" -pthread "$tmp/p.cpp" -o "$tmp/p" 2>/dev/null && ok "Pthreads (incl. pthread_barrier_t)" \
    ...
    "$CXX" -fopenmp "$tmp/o.cpp" -o "$tmp/o" 2>/dev/null && ok "OpenMP (-fopenmp)" \
        || miss "OpenMP not supported by $CXX (Apple clang++? see GETTING_STARTED.md)"
else
    miss "$CXX not found"
fi
...
if command -v "$MPICXX" >/dev/null; then
    ok "$MPICXX -> $($MPICXX --version 2>/dev/null | head -1)"
    command -v mpirun >/dev/null && ok "mpirun: $(mpirun --version 2>&1 | head -1)" || miss "mpirun not found"
else
    miss "$MPICXX not found (MPI days 10-20 need it)"
fi
