...
```bash
sudo dnf install -y gcc-c++ make openmpi openmpi-devel environment-modules
...
ls "$(brew --prefix)"/bin/g++-*              # find the version, e.g. g++-14
export CXX=g++-14                            # used by scripts/compile.sh and make
export OMPI_CXX=g++-14                       # makes Open MPI's mpicxx use GCC too (needed for MPI+OpenMP examples)
```
...
export MPICXX=mpiicpx                         # Intel MPI + icpx (older installs: mpiicpc)
```
...
sbatch jobs/slurm.sbatch mpi/day14-reduce-allreduce/arrSum_reduce.cpp

# PBS Pro / Torque
qsub -v SRC=mpi/day14-reduce-allreduce/arrSum_reduce.cpp jobs/pbs.sh
```
...
|---|---|---|
| `CXX` | `g++` | `CXX=clang++`, `CXX=g++-14`, `CXX=icpx` |
| `MPICXX` | `mpicxx` | `MPICXX=mpiicpx` |
| `CXXFLAGS` | `-O2 -Wall` | `CXXFLAGS="-O3 -march=native"` |
...
| Symptom | Fix |
|---|---|
| `mpicxx: command not found` | MPI not installed or not loaded — see your section above |
