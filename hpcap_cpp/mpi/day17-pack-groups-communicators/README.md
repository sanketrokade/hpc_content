# Day 17 — MPI: Pack/Unpack, Probe, Groups & Communicators

[← Day 16 — MPI: Derived Datatypes](../../mpi/day16-derived-datatypes/) · [Index](../../README.md) · [Day 18 — MPI: Pi, Prime Count, Matrix Addition](../../mpi/day18-pi-primes-matrix/)
...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        ...
        MPI_Pack(&data.cpp, 1, MPI_CHAR, buffer, buffer_size, &position, MPI_COMM_WORLD);
        ...
        MPI_Unpack(buffer, buffer_size, &position, &data.cpp, 1, MPI_CHAR, MPI_COMM_WORLD);
...
- Compile the program:
  ...
  ```bash
  ../../scripts/compile.sh mpi_pack_unpack.cpp
  ```

- Run the program:

  ```bash
  ../../scripts/run.sh ./mpi_pack_unpack.out 2
  ```
...
- Compile the program:
  ...
  ```bash
  ../../scripts/compile.sh mpi_pack_size.cpp
  ```

- Run the program:

  ```bash
  ../../scripts/run.sh ./mpi_pack_size.out 2
  ```
...
- Compile the program:
  ...
  ```bash
  ../../scripts/compile.sh mpi_probe_get_count.cpp
  ```

- Run the program:

  ```bash
  ../../scripts/run.sh ./mpi_probe_get_count.out 2
  ```
...
```bash
../../scripts/compile.sh mpi_any_src_and_tag.cpp
```

```bash
../../scripts/run.sh ./mpi_any_src_and_tag.out 10
```
...
- Compile the program:
  ...
  ```bash
  ../../scripts/compile.sh mpi_groups_communicators.cpp
  ```

- Run the program:

  ```bash
  ../../scripts/run.sh ./mpi_groups_communicators.out 5
  ```
...
int main(int argc, char** argv) {
    ...
    int *even_ranks = static_cast<int*>(malloc(half_size * sizeof(int)));
    int *odd_ranks = static_cast<int*>(malloc((size - half_size) * sizeof(int)));
...
- Compile the program:
  ...
  ```bash
  ../../scripts/compile.sh mpi_task_parallelism_manual_groups.cpp
  ```

- Run the program:

  ```bash
  ../../scripts/run.sh ./mpi_task_parallelism_manual_groups.out 9
  ```
...
- Compile the program:
  ...
  ```bash
  ../../scripts/compile.sh mpi_task_parallelism.cpp
  ```

- Run the program:

  ```bash
  ../../scripts/run.sh ./mpi_task_parallelism.out 10
  ```
...
```bash
../../scripts/compile.sh taskp.cpp
```

```bash
../../scripts/run.sh ./taskp.out 10
```
