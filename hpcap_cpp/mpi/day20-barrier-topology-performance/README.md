# Day 20 — MPI: Barrier, Topologies, Performance

[← Day 19 — MPI: Matrix Multiplication, MPI_Init_thread](../../mpi/day19-matrix-mult-init-thread/) · [Index](../../README.md)
...
1.  Timing Functions:
    ...
    end_time = MPI_Wtime(); printf("Execution time: %f seconds", end_time - start_time);
...
4.  Performance Analysis Tools: Tools like Paraver, Vampir, HPCToolkit, and Scalasca can be used to visualize and analyze the performance of MPI programs.
...
3.  Example Code
    ...
    ```bash
    ../../scripts/compile.sh mpi_cart.cpp
    ```

    ```bash
    ../../scripts/run.sh ./mpi_cart.out 2
    ```
...
```bash
../../scripts/compile.sh barrier.cpp
```

```bash
../../scripts/run.sh ./barrier.out 5
```
