# Day 3 — Pthreads: Reduction, Condition Variables, Barriers

[← Day 2 — Pthreads: Race Conditions & Mutex](../../pthreads/day02-race-conditions-mutex/) · [Index](../../README.md) · [Day 4 — OpenMP: Introduction](../../openmp/day04-openmp-intro/)
...
int main(){
    ...
    t = static_cast<pthread_t*>(malloc(sizeof(pthread_t) * T));
...
void *hello(void* threadId) {
    long tid = (long)threadId;
    long *localSum = static_cast<long*>(malloc(sizeof(long))); // Allocate memory for the local sum
    *localSum = 0;
    int chunk_size = N / T;
    int start = tid * chunk_size;
    int end = (tid + 1) * chunk_size;
...
void* hello(void*){
    long*localValue = static_cast<long*>(malloc(sizeof(long)));
    *localValue = 123;
...
```bash
g++ serialArrayAddition.cpp -o serialArrayAddition.out
```
...
```bash
g++ parallelArrayAddition.cpp -o parallelArrayAddition.out -lpthread
```
...
```bash
g++ parallelDataWithComputation.cpp -lpthread
```
